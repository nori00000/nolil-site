...
검토 커밋: `5d01d5428862c4d2c2ffb86338088886c6b0f97f` (2026-08-21 KST)

선정한 6개 `SKILL.md`를 읽고 `rm -rf`, credential 수집, 자동 게시·배포 지시를 확인하지 않았습니다. 다만 각 스킬이 인터넷으로 외부 사이트를 읽으므로, 프로젝트의 `.agents/skills/` 소유권과 실행 경계를 정한 뒤 SHA 고정 설치합니다. 현재 커밋에서는 외부 지침을 자동 로드하지 않고 이 문서와 `marketing-context.md`만 품질 계약으로 사용합니다.
